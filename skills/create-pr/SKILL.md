...
```
IF $ARGUMENTS に `review-skip` が指定されている
  → レビュースキップモード（軽微修正フロー向け）
ELSE
  → 通常モード
```
...
1. レビュースキップモードの場合は `$DIR/review-report.md` の確認をスキップし、そのまま Step 2 へ進む
2. 通常モードの場合は `$DIR/review-report.md` を読み込む
3. 総合判定を確認する:
   - **承認** → Step 2 へ進む
   - **承認以外** → 「レビューが未承認です（総合判定: 〇〇）。続行しますか？」とユーザーに確認する
...
1. `feat/<feature>/<具体名>` ブランチのローカル・リモートでの存在を確認する
   - `<具体名>` はPRグループ名をkebab-caseに変換する（例: `データモデルとAPI` → `data-model-and-api`）
   - 既存ブランチがある場合はそのブランチを使用する
   - 存在しない場合は `feat/<feature>` から作成する
2. `gh pr list --head <branch>` で既存PRの有無を確認する
3. 対応するタスクのコミットが含まれていることを確認する
4. リモートにプッシュ済みか確認し、未プッシュならプッシュする
5. `feat/<feature>/<具体名>` → `feat/<feature>` の子PRを作成または更新する:
   - 既存PRがある場合はタイトル・本文を更新する
   - 既存PRがない場合は新規作成する
   - タイトル: `<prefix>: <PRグループ名>`
   - 本文: 通常のPRテンプレートに準拠（対象タスクの範囲に限定）
   - ラベル: 通常の手順と同様
6. 子PRをマージする（次のPRグループの作業前にマージを完了する）
