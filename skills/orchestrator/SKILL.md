...
| 変更ファイル数見込み | 影響範囲を調査し、3ファイル以下で収まるか確認 |
| インターフェース変更の有無 | 公開API / props / schema / CLI / 設定キー等の変更有無を確認 |
...
```
IF 以下の条件をすべて満たす → 軽微修正フロー（Step 4f）
  - 変更内容が以下のいずれかに該当する
    - テキスト・文言の変更
    - スタイル・見た目の調整
    - 変数名・表示名のリネーム
    ...
    - 既存機能の挙動を変えないバグ修正
  - 変更ファイル数が3以下で収まる
  - 公開API / props / schema / CLI / 設定キー等のインターフェース変更がない

ELSE → 通常フロー（以下の判定に進む）
```

軽微修正フローの開始後に条件を超えることが判明した場合は、通常フローにエスカレーションする。
...
- 各ドキュメント生成後のユーザー確認はスキップする（Step 3 で全体計画の承認を得ているため）
...
- 単一PRの場合は `feat/<feature>` ブランチで未完了タスクを依存順に処理する
- 分割PRの場合は PRグループ順に処理し、各グループの開始時に `feat/<feature>/<具体名>` ブランチを `feat/<feature>` から切る
- タスクごとに: 実装 → テスト実行（Green） → リファクタリング → テスト再実行 → タスク完了記録 → コミット
- 分割PRの場合は各PRグループ完了後、そのブランチを対象に 4d → 4e を実行して子PRを作成・マージし、次のPRグループへ進む
...
実行前および実行中に、変更ファイル数が3を超える、または公開API / props / schema / CLI / 設定キー等のインターフェース変更が必要と判明した場合は、軽微修正フローを中止して通常フローにエスカレーションする。

1. **ブランチ作成**
   - mainブランチから適切なプレフィックス付きのブランチを作成する
   - プレフィックスは変更内容に応じて選択する（`fix/`, `refactor/`, `style/` 等）
   - 例: `fix/rename-todo-to-task`, `style/update-button-color`
...
5. **コミット・プッシュ・PR作成**
   ...
   - リモートにプッシュする
   - `create-pr` スキルをレビュースキップモードで実行し、`review-report.md` の確認をスキップしてPRを作成する
...
**注意:**
- 軽微修正フローではドキュメント更新（PRD・仕様書等）、レビューはスキップする
...
**分割PRの場合:**
- PRグループごとに 4c → 4d → 4e を繰り返す
- 4e では、4c で実装済みの `feat/<feature>/<具体名>` ブランチを対象に `feat/<feature>` への子PRを作成または更新する
- 子PRをマージ後、次のPRグループがある場合は更新後の `feat/<feature>` を基点に次の `feat/<feature>/<具体名>` ブランチで 4c から再開する
